...
import { CustomerSignUpResponseDto, StoreSignUpResponseDto } from './dtos/resonse_dtos/signup_response.dto';
...
export class AuthService {
    constructor(private store_repository: StoreRepository, private jwt_service: JwtService, private customer_repository: CustomerRepository) { }
    ...
    async customer_signup(create_customer_dto: CreateCustomerDto): Promise<CustomerSignUpResponseDto> {
        try {
            ...
            create_customer_dto.password = await hashPassword(create_customer_dto.password)

            const inserted_customer = await this.customer_repository.create_customer(create_customer_dto)
    ...
    async customer_signin(customer_signin_dto: CustomerSignInDto): Promise<CustomerSignUpResponseDto> {
        try {
            const customer = await this.customer_repository.get_customer_by_email(customer_signin_dto.email)

            if (!customer?.email) {
                throw new BadRequestException('Incorrect email or password')
            }

            const isPasswordValid = await comparePassword(customer_signin_dto.password, customer.password)
