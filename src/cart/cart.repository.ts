...
export class CartRepository {
    ...
    async get_cart_by_id(_id: Types.ObjectId, projection?: CartProjection) {
        return this.cart_model.findOne({ _id }, projection).populate("cart_items").exec()
    }


    async get_cart_by_customer_id(customer: Types.ObjectId, projection?: CartProjection) {
        return this.cart_model.findOne({ customer }, projection).exec()
    }
    ...
    async update_cart_by_customer_id(customer: Types.ObjectId, cart_dto: UpdateCartDto, session?: ClientSession | null) {
        if (!session) session = null

        return this.cart_model.updateOne(
            { customer },
            cart_dto,
