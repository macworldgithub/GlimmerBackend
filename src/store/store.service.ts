...
export class StoreService {
    ...
    async get_sales(store_payload: AuthPayload, month: number, year: number): Promise<any> {
        try {
            ...
            monthly_sales.map(elem => {
                elem.order_items.map((ord) => {
                    if (!(ord instanceof Types.ObjectId)) {
                        sum = sum + (ord.product.discounted_price ? ord.product.discounted_price : ord.product.base_price)
                    }
                })
            });
            return sum
