...
export class OrderRepository {
    constructor(@InjectModel(Order.name) private order_model: Model<Order>, @InjectModel(OrderItem.name) private order_item_model: Model<OrderItem>, @InjectModel(StoreOrder.name) private store_order_model: Model<StoreOrder>) { }
    ...
    async create_many_order_items(order_items: CreateOrderItem[], session?: ClientSession) {
    ...
    async update_order_by_id(id: Types.ObjectId, order_dto: UpdateOrderDto, session ?: ClientSession | null) {
    ...
    async create_many_store_orders(store_orders: StoreOrder[], session ?: ClientSession) {
        return this.store_order_model.insertMany(store_orders, {session})
    }

    async get_store_order_by_id(_id: Types.ObjectId, projection?: OrderProjection) {
        return this.store_order_model.findOne({ _id }, projection).populate(["order_items", "order", "store"]).exec()
    }
    ...
    async update_store_order_by_id(id: Types.ObjectId, store_order_dto: UpdateStoreOrder) {
