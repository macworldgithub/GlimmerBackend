...
export class OrderService {
    ...
    async create_order(order_dto: OrderReqDto, user: AuthPayload) {
        ...
        try {
            ...
            await this.order_repository.create_many_store_orders(
                store_orders_obj,
                session,
            );
    ...
    async get_all_store_orders(page_no: number) {
        try {
            const orders = await this.order_repository.get_all_store_orders(page_no)
    ...
    async get_store_order_by_id(id: string) {
        try {
            const order = await this.order_repository.get_store_order_by_id(new Types.ObjectId(id))
    ...
    async update_store_order_by_id(id: string, update_store_order_dto: UpdateStoreOrder) {
        try {
            const order = await this.order_repository.update_store_order_by_id(new Types.ObjectId(id), update_store_order_dto)
