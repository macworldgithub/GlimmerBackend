...
export class ProductService {
  ...
  async delete_store_product_by_id(id: string): Promise<DeleteResponse> {
    try {
      const product =
  ...
  async bulk_update_product_prices(
    ...
    try {
      const bulkWriteResult =
        await this.product_repository.bulk_update_product_prices(
          discount,
          productIds,
        );
  ...
  async get_all_products(
    ...
    try {
      ...
      if (category) {
        ...
        console.log(`Category slug: ${category}, Result:`, categoryDoc); // Debugging
        if (!categoryDoc) {
  ...
  async submit_product_rating(
    ...
    try {
      ...
      const existingRating = await this.rating_repository.get_user_rating(
        productId,
        userId,
      );
      if (existingRating) {
      ...
      const updatedProduct =
        await this.product_repository.update_product_rating(productId, update);
  ...
  async get_product_rating(product_id: string): Promise<{
    ...
    try {
      const stats = await this.rating_repository.get_rating_stats(
  ...
  > {
    try {
      const ratings = await this.rating_repository.get_product_ratings(
        new Types.ObjectId(product_id),
      );
      ...
      throw new InternalServerErrorException(
        'Failed to retrieve product ratings',
      );
    }
  }
  ...
  async update_product_rating(
    ...
    try {
      ...
      const updatedProduct =
        await this.product_repository.update_product_rating(
          ratingObj.productId,
          update,
        );
  ...
  async get_store_product_count(store_payload: AuthPayload): Promise<number> {
    try {
      ...
      console.error(e);
      throw new InternalServerErrorException(
        'Failed to retrieve store product count',
      );
    }
  }
  ...
  async get_all_rated_products(
    ...
    try {
      ...
      const products =
        await this.product_repository.get_all_rated_products(pipeline);
      ...
      const totalResult =
        await this.product_repository.get_all_rated_products(totalPipeline);
